...
The near-leakage category predictors `cat_game_demo` and
`cat_downloadable_content` are excluded from model predictors before the
correlation diagnostics are calculated.

```{r}
#| label: tbl-validation-summary
#| tbl-cap: "Data validation check summary by pipeline stage."
validation_report_paths <- c(
  "../data/data_validation_report.csv",
  "../results/train_validation_report.csv"
)

existing_validation_reports <- validation_report_paths[file.exists(validation_report_paths)]

validation_summary <- if (length(existing_validation_reports) > 0) {
  existing_validation_reports |>
    purrr::map_dfr(read_csv, show_col_types = FALSE) |>
    count(stage, status, name = "n_checks") |>
    arrange(stage, status)
} else {
  tibble(stage = "validation_reports", status = "missing", n_checks = 0L)
}
