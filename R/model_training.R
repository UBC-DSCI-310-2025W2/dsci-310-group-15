...
select_model_predictors <- function(data, category_pattern = "^cat_") {
  ...
  processandplot::validate_required_columns(data, predictors, "data")
  predictors
}
...
create_stratified_train_test_split <- function(
    ...
    train_prop = 0.8,
    seed = 123) {
  processandplot::validate_required_columns(data, target_col, "data")
...
evaluate_logistic_model <- function(
    ...
    target_col = "is_free",
    positive_class = "Free") {
  processandplot::validate_required_columns(test_model, target_col, "test_model")
...
run_train_test_model <- function(
  ...
  processandplot::ensure_directory_exists(results_dir, "results_dir")
  ...
  output_paths <- list(
    feature_importances = processandplot::build_file_path(results_dir, "feature_importances_table.csv"),
    evaluation_metrics = processandplot::build_file_path(results_dir, "evaluation_metrics_table.csv"),
    training_validation = processandplot::build_file_path(results_dir, "train_validation_report.csv"),
    roc_curve = processandplot::build_file_path(results_dir, "roc_curve.png"),
    confusion_matrix = processandplot::build_file_path(results_dir, "confusion_matrix.png")
  )
